    ...
    public function run(): void
    {
        AiPrompt::updateOrCreate(
            ...
            [
                ...
                'temperature' => null,
                'system_prompt' => null,
                'active' => false, // do not clobber whichever version activate() has set live
                'template' => <<<'PROMPT'
...
PROMPT,
        ...
        // v3 — fixes prompt gaps identified in production:
        // - Targets/thresholds must NEVER appear as chart data points (Bug #2)
        // - Chart descriptions must be consistent with actual data arrays (Bug #3)
        AiPrompt::updateOrCreate(
            ['name' => 'document_insights', 'version' => 3],
            [
                'provider' => 'anthropic',
                'model' => config('services.anthropic.model'),
                'temperature' => null,
...
PROMPT,
        ...
        // Activate v3 as the current production version
        AiPrompt::activate('document_insights', 3);
    }
}